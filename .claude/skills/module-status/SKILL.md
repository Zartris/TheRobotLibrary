---
name: module-status
description: Summarize implementation and milestone status for a module or the whole workspace. Pass a module name to focus, or no args for a workspace-wide overview.
...
For workspace overview, group by domain (common, control, perception, state_estimation, motion_planning, fleet_management) and list each module on one line with emoji status:
